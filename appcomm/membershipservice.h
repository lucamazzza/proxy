 ...
 * @brief Utilities for topic membership mapping and normalization.
 *
 ...
 * topic membership creation and parsing.
 */
...
public:
     ...
     * @return A TopicMember marked as active.
     */
    model::TopicMember createActiveMember(const QString &topicId,
                                            const QString &userId,
                                            const QDateTime &joinedAt = QDateTime::currentDateTimeUtc()) const;

    /*!
     * @brief Parses and filters topic-member documents.
     * @param documents Raw Appwrite documents.
     * @return Valid parsed members with non-empty user and topic identifiers.
     */
    QList<model::TopicMember> parseMembers(const QJsonArray &documents) const;
