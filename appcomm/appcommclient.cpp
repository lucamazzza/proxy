...
{
    if (!d->m_clientState.authenticated && isSessionResponse(data)) {
        d->m_sessionInfo.sessionId = data.value("$id").toString().trimmed();
        d->m_sessionInfo.userId = data.value("userId").toString().trimmed();
        d->m_sessionInfo.authType = (data.value("provider").toString().trimmed().toLower() == "email")
